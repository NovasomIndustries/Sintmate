...
uint16_t	range_ar[NUM_MEASURES];
...
void SintMate_VL53L0X_StartMeasure(void)
{
	...
	range = 0;
	range_index = NUM_MEASURES;
	VL53L0X_StartReadRangeSingleIRQ();
}

void VL53L0X_ReadRangeSingleIRQ_Callback(void)
{
uint32_t	lrange=0,arange,i;

	arange =  VL53L0X_readReg16Bit(RESULT_RANGE_STATUS + 10);
	range_ar[range_index-1] = ((arange >> 8 ) & 0xff) | ((arange << 8)&0xff00);
